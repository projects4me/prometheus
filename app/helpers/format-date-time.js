/*
 * Projects4Me Copyright (c) 2017. Licensing : http://legal.projects4.me/LICENSE.txt. Do not remove this line
 */

import { inject as service } from '@ember/service';
import Helper from '@ember/component/helper';

/**
 * Formats a date time string according to the specified format.
 *
 * @class FormatDateTimeHelper
 * @namespace Prometheus.Helpers
 * @extends Helper
 * @author Rana Nouman <ranamnouman@gmail.com>
 */
export default class FormatDateTimeHelper extends Helper {
	/**
	 * The current authenticated user of the application.
	 *
	 * @property currentUser
	 * @type Ember.Service
	 * @for FormatDateTimeHelper
	 */
	@service currentUser;

	/**
	 * The default format for the date time.
	 *
	 * @property format
	 * @type string
	 * @for FormatDateTimeHelper
	 */
	format = "DD MMM 'YY, h:mm a";

	/**
	 * Computes the formatted date time string
	 *
	 * @param {Array} _positional - Unused positional parameters.
	 * @param {Object} options - The options object.
	 * @param {string|Date} options.date - The date to format.
	 * @param {string} [options.format] - The format string to use (follows Moment.js format).
	 * @param {boolean} [options.humanize=false] - If true, returns a relative time string (e.g. "2 days ago").
	 * @returns {string} The formatted date time string or empty string if date is undefined.
	 */
	compute([], { date, format, humanize = false }) {
		let formattedDate = '';
		format = format || this.format; // prefer the format passed in, otherwise use the default
		let userTimezone = this.currentUser.user.timezone;
		let momentDate;

		if (date !== undefined) {
			momentDate = moment.utc(date).tz(userTimezone);
			formattedDate = humanize
				? momentDate.fromNow()
				: momentDate.format(format);
		}

		return formattedDate;
	}
}
